...
for rok in range(aktualni_datum_a_cas.year, (aktualni_datum_a_cas.year+5)):
    datum_velikonoc = easter.easter(rok)
    print(f"Velikonoční neděle v roce {rok}: {datum_velikonoc.strftime('%d.%m.%Y')}")
...
while True:
        ...
        if calendar.weekday(rok, 12, 24) == 6:
            print(f"Štědrý den bude v neděli v roce {rok}.")
            break
        rok += 1